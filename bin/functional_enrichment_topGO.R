...
lapply(dge_files, function(res_f){
    ...
    if (length(genes_up)>=5){
        ...
        resultsFisher_up <- runTest(
            GOData_up, algorithm = "weight01", statistic = "fisher")

        allRes_up <- GenTable(
            GOData_up, p_value = resultsFisher_up,
        ...
        go_up_plot[["order"]] <- factor(
            1:nrow(go_up_plot), levels=rev(1:nrow(go_up_plot)))
    ...
    if (length(genes_down)>=5){
        ...
        resultsFisher_down <- runTest(
            GOData_down, algorithm = "weight01", statistic = "fisher")
        allRes_down <- GenTable(
            GOData_down, p_value = resultsFisher_down,
        ...
        go_down_plot[["order"]] <- factor(
            1:nrow(go_down_plot), levels=rev(1:nrow(go_down_plot)))
