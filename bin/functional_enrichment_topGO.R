...
lapply(dge_files, function(res_f){
    ...
    term_genes_list_up <- term_genes_list_up[lapply(term_genes_list_up,length)>0]
    term_genes_list_down <- term_genes_list_down[lapply(term_genes_list_down,length)>0]


    ## make factor coding for upregulated/downregulated genes
    ...
    #create topGO object
    GOData_up <- new(
        "topGOdata", ontology = "BP", allGenes = upList, nodeSize = min_size,
        annot = annFUN.gene2GO, gene2GO = gene_terms_list)

    GOData_down <- new(
        "topGOdata", ontology = "BP", allGenes = downList, nodeSize = min_size,
        annot = annFUN.gene2GO, gene2GO = gene_terms_list)

    #run test
    resultsFisher_up <- runTest(GOData_up, algorithm = "weight01", statistic = "fisher")
    allRes_up <- GenTable(
        GOData_up, p_value = resultsFisher_up, topNodes = length(usedGO(GOData_up)))

    resultsFisher_down <- runTest(GOData_down, algorithm = "weight01", statistic = "fisher")
    allRes_down <- GenTable(
        GOData_down, p_value = resultsFisher_down, topNodes = length(usedGO(GOData_down)))


    ########################################
    ## annotate which genes are in each Term
    ########################################
    allRes_up <- data.frame(do.call(rbind, lapply(1:nrow(allRes_up), function(idx){
        row = allRes_up[idx,]
        term = row[["GO.ID"]]
    ...
    go_up_plot[["order"]] <- factor(1:nrow(go_up_plot), levels=rev(1:nrow(go_up_plot)))
    go_down_plot[["order"]] <- factor(1:nrow(go_down_plot), levels=rev(1:nrow(go_down_plot)))

    go_up_plot[["p_value"]] <- as.numeric(go_up_plot[["p_value"]])
    go_down_plot[["p_value"]] <- as.numeric(go_down_plot[["p_value"]])

    p1 <- ggplot(go_up_plot,
